...
tmp_file="./tmp.tmp"
target_file="./pkg_def_gen_by_tool.h"
let total_tid=0
let line_num=0
...
echo "//////////////////////////////////////////////////" >> $target_file
echo '#ifndef __PKG_DEF_H__' >>  $target_file
echo '#define __PKG_DEF_H__' >>  $target_file

echo "" >>  $target_file
echo '#include "fld_def.h"'  >>  $target_file
echo '#include "pkg_base.h"' >>  $target_file
echo "" >>  $target_file
echo '#define DECL_TID(id) enum{TID = id}' >>  $target_file
echo '#define DECL_LABEL() \'              >>  $target_file
echo 'public: \'                           >>  $target_file
echo '    uint32_t label_{0}; \'           >>  $target_file
echo '    uint32_t get_label() \'          >>  $target_file
echo '    { \'                             >>  $target_file
echo '    	return label_; \'              >>  $target_file
echo '    }'                               >>  $target_file
echo '#define DECL_LABEL_MOCK() \'         >>  $target_file
echo 'public: \'                           >>  $target_file
echo '    uint32_t get_label() \'          >>  $target_file
echo '    { \'                             >>  $target_file
